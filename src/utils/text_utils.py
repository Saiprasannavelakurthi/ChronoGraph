...
def normalize_entity_name(name: str) -> str:
    ...
    Preserves case-sensitive identifiers like GitHub usernames, commit hashes,
    and Jira keys, while standardizing whitespace and title casing for general text.
    """
    if not name:
        return ""
    ...
    # Do not mutate commit hashes or Jira issue keys
    if COMMIT_HASH_PATTERN.match(raw_name) or JIRA_KEY_PATTERN.match(raw_name) or PR_ISSUE_NUM_PATTERN.match(raw_name):
        return raw_name
    ...
    # Basic normalization for names: strip surrounding punctuation
...
def generate_entity_id(name: str, entity_type: str) -> str:
    ...
    Example: name="Karkuvel", type="PERSON" -> "person_karkuvel"
    """
    type_prefix = str(entity_type).lower().replace("entitytype.", "")
...
def deduplicate_entities(entities: List[Entity]) -> List[Entity]:
    ...
    Normalizes names and IDs across occurrences.
    """
    ...
    for entity in entities:
        ...
        # Calculate canonical id if missing or generic
        entity_id = entity.id.strip() if entity.id else generate_entity_id(norm_name, type_str)

        # Check if we already have this entity by name (case-insensitive) or ID
        name_key = f"{type_str}:{norm_name.lower()}"
