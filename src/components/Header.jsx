import { Menu, Circle, PanelRight } from "lucide-react";

export default function Header({ title, onToggleSidebar, onTogglePanel, panelOpen }) {
  return (
    <header className="flex items-center justify-between border-b border-mist-200 bg-white/80 px-4 py-3.5 backdrop-blur">
      ...
      >
        <PanelRight size={14} />
        <span className="hidden sm:inline">{panelOpen ? "Hide panel" : "View panel"}</span>
      </button>
    </header>
  );
}
